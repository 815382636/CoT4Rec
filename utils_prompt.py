def construct_source(history, preference=None, rec_list=None):
    if not preference:
        # 没有使用用户偏好
...
def construct_pre_source(history):
        ...
        if form == "REC-PA":
            prompt = (
                ...
                if stage == 1
                    ...
                    data["history"], data["pred_preference"], data["recommendations"]
                )
            )
        elif form == "REC-A":
            prompt = construct_source(data["history"], None, data["recommendations"])
        # target = construct_target(data["result"][:10])
        target = data["result"][0]
    # 训练用户偏好
    else:
        prompt = construct_pre_source(data["history"])
